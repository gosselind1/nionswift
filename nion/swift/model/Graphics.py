...
def scale_rectangle_like(part_name: str, data_shape: Geometry.FloatSize, bounds: Geometry.FloatRect, rotation: float,
    ...
    if modifiers.shift or "square" in constraints:
        max_size = max(abs(new_width), abs(new_height))
        square_width = max_size * (new_width / abs(new_width))
        square_height = max_size * (new_height / abs(new_height))
        width_modifier = new_width - square_width
        height_modifier = new_height - square_height
        new_width = square_width
        new_height = square_height
        new_origin_x += width_modifier * x_percent
        new_origin_y += height_modifier * y_percent
